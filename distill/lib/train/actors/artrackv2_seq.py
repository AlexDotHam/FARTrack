...
class ARTrackV2SeqActor(BaseActor):
    ...
    def batch_init(self, images, template_bbox, initial_bbox) -> dict:
        ...
        template_bbox_list = []
        number = template_bbox.shape[1]
        #print(template_bbox.shape)
        for i in range(number):  # 处理7个模板
            template_bbox_list.append(bbutils.batch_xywh2center2(template_bbox[:, i]))  # ndarray:(num_seq,4)

        initial_bbox = bbutils.batch_xywh2center2(initial_bbox)  # ndarray:(num_seq,4)
        self.center_pos = initial_bbox[:, :2]  # ndarray:(num_seq,2)
        self.size = initial_bbox[:, 2:]  # ndarray:(num_seq,2)
        ...
        s_z_list = []
        for template_bbox in template_bbox_list:
            w_z = template_bbox[:, 2] * template_factor  # ndarray:(num_seq)
            h_z = template_bbox[:, 3] * template_factor  # ndarray:(num_seq)
            s_z = np.ceil(np.sqrt(w_z * h_z))  # ndarray:(num_seq)
            s_z_list.append(s_z)

        # 计算通道平均值
        self.channel_average = []
        for img in images:
            for i in range(number):  # 对每个模板图像计算平均值
                self.channel_average.append(np.mean(img[i], axis=(0, 1)))
        self.channel_average = np.array(self.channel_average)  # ndarray:(7*num_seq,3)

        # get crop
        z_crop_list = []
        for i in range(len(images)):  # 遍历每个序列
            sequence_crops = []
            for j in range(number):  # 处理7个模板
                here_crop = self.get_subwindow(images[i][j], template_bbox_list[j][i, :2],
                                             self.cfg.DATA.TEMPLATE.SIZE, s_z_list[j][i],
                                             self.channel_average[number * i + j])
                ...
                sequence_crops.append(z_crop.clone())  # 添加维度并克隆
            sequence_tensor = torch.concat(sequence_crops, dim=0)
            z_crop_list.append(sequence_tensor.unsqueeze(0).clone())  # 存储当前序列的所有模板crop
        z_crop_tensor = torch.concat(z_crop_list, dim=0)
        out = {'template_images': z_crop_tensor}
        return out
    ...
    def batch_track(self, img, gt_boxes, template, action_mode='max') -> dict:
      ...
      #  target_in_search = torch.cat(target_in_search_list, dim=0)
        ...
        outputs = self.net(template, x_crop, seq_input=pre_seq_output, head_type=None,
                           stage="batch_track",)
      ...
      #  dz_feat_udpate_list = []
        ...
        for idx in range(np.max(num_frames)):
            ...
            if idx == 0:
                ...
                self.template_temp = outputs_template['template_images']
               # z_all = [outputs_template['z_1'], outputs_template['z_2']]
               # results['z_all'] = z_all
             #   self.dz_feat_update = outputs_template['z_2_feat']

            else:
                outputs = self.batch_track(here_images, here_gt_bbox, self.template_temp,
                                           action_mode='half')
                if outputs == None:
                    return None
     ...
     #   dz_feat_update_reverse_list = []
        ...
        for idx in range(np.max(num_frames)):
            ...
            if idx == 0:
                outputs_template = self.batch_init(template, template_bbox, here_gt_bbox)
                results['template_images'] = outputs_template['template_images']
                self.template_temp = outputs_template['template_images'].clone()
             #   z_all = [outputs_template['z_1'], outputs_template['z_2']]
            #    results['z_all'] = z_all
    ...
    def compute_sequence_losses(self, data):
     ...
     #   target_in_search = data['target_in_search'].reshape(-1, *data['target_in_search'].size()[2:])
        ...
        if epoch < 21:
            ...
            self.loss_weight['score_update'] = 0
        #elif epoch < 31:
        #    self.loss_weight['focal'] = 0
        ...
        outputs = self.net(template_images_for, search_images, seq_input=pre_seq, stage="forward_pass",)
        extra_pred_list = outputs['extra_list']
        
        verifocal_extra_loss_list = []
        ciou_extra_loss_list = []
        beta = 1
        
        pred_feat = outputs["feat"]
        
        if self.focal == None:
            weight = torch.ones(self.bins * self.range + 5) * 1
        ...
        for i in range(len(extra_pred_list)):
            #search_anno[:, 2] = search_anno[:, 2] + search_anno[:, 0]
           # search_anno[:, 3] = search_anno[:, 3] + search_anno[:, 1]
            ...
            pred = extra_pred_list[i].permute(1, 0, 2).reshape(-1, self.bins * self.range + 5)
            #target = seq_output.reshape(-1).to(torch.int64)

            varifocal_loss = self.focal(pred, target)
            verifocal_extra_loss_list.append(varifocal_loss)

            pred = extra_pred_list[i][0:4, :, 0:self.bins * self.range] * beta
            ...
            cious = cious.mean()
            ciou_extra_loss_list.append(cious)
       ...
       # score_loss = self.objective['l1'](score_real, iou)
        ...
        ciou_extra_loss_list.append(giou_loss)
        verifocal_extra_loss_list.append(varifocal_loss)

        

        total_losses = self.loss_weight['giou'] * torch.mean(torch.stack(ciou_extra_loss_list)) + self.loss_weight[
            'focal'] * torch.mean(torch.stack(verifocal_extra_loss_list)) #+ self.loss_weight['score'] * score_loss * 0

        mean_iou = iou.detach().mean()
        status = {
                "Loss/total": total_losses.item() / 2,
            ...
            # 添加每个giou loss
        for i in range(len(ciou_extra_loss_list)):
            status[f"Loss/giou_{i}"] = ciou_extra_loss_list[i].item()/2
        status["Loss/giou_mean"] = torch.mean(torch.stack(ciou_extra_loss_list)).item() / 2

            # 添加每个location loss
        for i in range(len(verifocal_extra_loss_list)):
            status[f"Loss/location_{i}"] = verifocal_extra_loss_list[i].item() /2
        status["Loss/location_mean"] = torch.mean(torch.stack(verifocal_extra_loss_list)).item() / 2
