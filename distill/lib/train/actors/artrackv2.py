...
#        assert len(data['template_images']) == 5
        ...
        seq_input = torch.cat([torch.ones((B, 1)).to(search_img) * x0, torch.ones((B, 1)).to(search_img) * y0,
                  ...
                  #             torch.ones((B, 1)).to(search_img) * score
                               ], dim=1)
    ...
    def compute_inter_layer_kl(self, layer_features):
    #"""
   # 计算相邻层坐标token特征之间的KL散度损失
    
   # 参数:
  #      layer_features (List[torch.Tensor]): 
   #         各层的坐标token特征列表，每个张量形状为 [B,4,D]
  #          顺序为从浅层到深层（layer0, layer1, ..., layerN-1）
    
  #  返回:
  #      List[torch.Tensor]: 相邻层间的KL散度损失列表，长度为N-1
 #   """
        kl_losses = []
    
    # 遍历每对相邻层（上层i+1 -> 下层i）
        for i in range(len(layer_features) - 1):
            upper_layer = layer_features[i+1].transpose(0, 1).detach()[:, :, :-5]  # 上层特征（梯度截断）
            lower_layer = layer_features[i].transpose(0, 1)[:, :, :-5]            # 下层特征
            #print(upper_layer.shape)
        # 将特征转换为概率分布
            teacher = F.softmax(upper_layer, dim=-1)  # 使用log_softmax
            student = F.log_softmax(lower_layer, dim=-1)          # 使用softmax
        
        # 计算KL散度 (KL(upper || lower))
            kl_loss = F.kl_div(
                input=student, 
                target=teacher,
                reduction='batchmean',  # 按batch维度求平均
                log_target=False        # target是概率分布而非log概率
            )
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        extra_pred_list = pred_dict['extra_list']
        if self.focal == None:
            weight = torch.ones(bins * self.range + 5) * 1
        ...
        for i in range(len(extra_pred_list)):
            pred = extra_pred_list[i].permute(1, 0, 2).reshape(-1, bins * self.range + 5)
            ...
            pred = extra_pred_list[i][0:4, :, 0:bins * self.range] * beta
            target = seq_output[:, 0:4].to(pred_feat)
            target_box = seq_output[:, 0:4].cpu().numpy()

            out = pred.softmax(-1).to(pred)
            # mul = torch.range((self.range * 0.5 * -1 + 0.5) + 1 / (self.bins * self.range),
            #                   (0.5 + self.range * 0.5), 2 / (self.bins * self.range)).to(
            #     pred)
            mul = torch.range((self.range * 0.5 * -1 + 0.5),
                              (0.5 + self.range * 0.5) - 1 / (self.bins * self.range), 2 / (self.bins * self.range)).to(
                pred)
            ans = out * mul
            ans = ans.sum(dim=-1)
        ...
        extra_pred_list.append(pred_feat)
        
        kl_loss_list = self.compute_inter_layer_kl(extra_pred_list)
        
        pred = pred_feat.permute(1, 0, 2).reshape(-1, bins * self.range + 5)
        target = seq_output.reshape(-1).to(torch.int64)
        ...
        #                   (0.5 + self.range * 0.5), 2 / (self.bins * self.range)).to(
        #     pred)
        mul = torch.range((self.range * 0.5 * -1 + 0.5), (0.5 + self.range * 0.5) - 1 / (self.bins * self.range), 2 / (self.bins * self.range)).to(pred)
        ans = out * mul
        ans = ans.sum(dim=-1)
        ans = ans.permute(1, 0).to(pred)
        ...
        loss = self.loss_weight['giou'] * torch.mean(torch.stack(ciou_extra_loss_list)) + self.loss_weight[
            'focal'] * torch.mean(torch.stack(verifocal_extra_loss_list))# + 0.01 * torch.mean(torch.stack(kl_loss_list))# + self.loss_weight['score'] * score_loss * 0
        ...
        if return_status:
            ...
            for i in range(len(ciou_extra_loss_list)):
                status[f"Loss/giou_{i}"] = ciou_extra_loss_list[i].item()
            status["Loss/giou_mean"] = torch.mean(torch.stack(ciou_extra_loss_list)).item()

            # 添加每个location loss
            for i in range(len(verifocal_extra_loss_list)):
                status[f"Loss/location_{i}"] = verifocal_extra_loss_list[i].item()
            status["Loss/location_mean"] = torch.mean(torch.stack(verifocal_extra_loss_list)).item()
            
           # for i in range(len(kl_loss_list)):
           #     status[f"Loss/KL_{i}"] = kl_loss_list[i].item()
          #  status["Loss/KL_mean"] = torch.mean(torch.stack(kl_loss_list)).item()
