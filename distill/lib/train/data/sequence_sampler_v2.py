...
class SequenceSampler(torch.utils.data.Dataset):
    ...
    def _sequential_sample(self, visible):
        ...
        for i in range(self.num_template_frames):
            template_frame_id = self._sample_visible_ids(visible, num_ids=1, min_id=0,
                                                      max_id=len(visible) - self.num_search_frames)
            template_frame_ids.append(template_frame_id[0])
