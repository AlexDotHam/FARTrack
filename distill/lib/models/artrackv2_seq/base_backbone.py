...
class BaseBackbone(nn.Module):
    ...
    def forward_features(self, z_0, x, identity, seqs_input):
   ...
   #     z_1 += identity[:, 1, :].repeat(B, self.pos_embed_z.shape[1], 1)
    ...
    #    x_feat = zxs[:, lens_z_single:lens_z_single+len_x]
     ...
     #   out_list.append(out.unsqueeze(0))
        ...
        output = {'seqs': seqs_output, 'class': values, 'feat': temp, "state": "val/test", "seq_feat": seq_feat.permute(1, 0, 2), "x_feat": x_feat, "extra_list": at_list}

        return output, None, None, None, None

    def forward(self, z_0, x, identity, seqs_input, **kwargs):
        """
        ...
        Returns:
            ...
            attn : None
        """
        output = self.forward_features(z_0, x, identity, seqs_input)
