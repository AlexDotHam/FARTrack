...
def generate_square_subsequent_mask(sz, sx, ss):
    ...
    mask[:, :] = 0
    mask[:int(sz), :int(sz)] = 1 #template self
...
class BaseBackbone(nn.Module):
   ...
   #     pos_x = self.word_embeddings.weight[200:600].unsqueeze(1).expand(-1, self.bins, -1)
    #    pos_y = self.word_embeddings.weight[200:600].unsqueeze(0).expand(self.bins, -1, -1)
     #   pos_2d = pos_x + pos_y
        
     #   pos_2d = pos_2d.permute(2, 0, 1).unsqueeze(0)
   #     downsampled = self.spatial_reduction(pos_2d)
        
   #     position_embeddings = downsampled.squeeze(0).permute(1, 2, 0)  # (H, W, dim)
    #    position_embeddings = position_embeddings.reshape(-1, 192).unsqueeze(0)
#
        ...
        z_0 = torch.stack(z_0, dim=1)  # [B, L, C, H, W]
        B, L = z_0.shape[:2]
  ...
  #      x += position_embeddings
      ...
      #  z_0 += identity[:, 0, :].repeat(B, self.pos_embed_z0.shape[1], 1)
       # z_1 += identity[:, 1, :].repeat(B, self.pos_embed_z1.shape[1], 1)
        ...
        at_list = []
        attention_list = []
        j_list = [4,5,6,7,8,9, 10, 11]

        for j, blk in enumerate(self.blocks):
            x = blk(x)
            if j in j_list:
                x_out = self.norm(x[:, -4:])
                 #x_out = self.norm_4(x[:, -5:-1])
            #
        ...
        at = at[:, -4:]
        at = at.transpose(0, 1)
        output = {'feat': at, 'score_feat':None, "state": "train", "extra_list": at_list, "attention_list": attention_list}
    ...
    #    z_1 += self.pos_embed_z1
     ...
     #   x += position_embeddings
      ...
      #  print("f2")
        ...
        for j, blk in enumerate(self.blocks):
            zxs, attn = blk(zxs, return_attention=True)
     ...
     #   z_1_feat = zxs[:, lens_z_single:lens_z_single * 2]
      #  x_feat = zxs[:, lens_z_single:lens_z_single + lens_x]
      ...
      #  print("f3")
        ...
        output = {'seqs': seqs_output, 'class': values, 'feat': temp, "state": "val/test",
                  "x_feat": output_x_feat.detach(), "score_feat": None, "attention_list": attention_list}
