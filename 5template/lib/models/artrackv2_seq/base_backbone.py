...
class BaseBackbone(nn.Module):
    ...
    #    score = self.bins * self.range + 5
        ...
        #eqs_input_ = torch.cat([trajectory, command], dim=1)
        seqs_input_ = command
   ...
   #     z_1 += identity[:, 1, :].repeat(B, self.pos_embed_z.shape[1], 1)
    ...
    #    x_feat = zxs[:, lens_z_single:lens_z_single+len_x]
     ...
     #   score_feat = zxs[:, -1]
        ...
        output = {'seqs': seqs_output, 'class': values, 'feat': temp, "state": "val/test", "seq_feat": seq_feat.permute(1, 0, 2), "x_feat": x_feat}
