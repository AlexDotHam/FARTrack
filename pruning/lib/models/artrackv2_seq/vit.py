...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attention=False, padding_mask=None):
       ...
       #     q, k, v,
       #     dropout_p=0.0,  # 推理时关闭dropout
       #     is_causal=False  # 根据实际情况调整
       # )
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.masknorm1 = MaskedLayerNorm(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        self.masknorm2 = MaskedLayerNorm(dim)
    ...
    def forward(self, x, return_attention=False, padding_mask=None):
        ...
        mask = padding_mask[:, 0, :]
        #print(mask)
     #   mask = torch.ones([B, 445]).to(x)
       # mask[:, 5:10] = 0
        if return_attention:
            feat, attn = self.attn(self.masknorm1(x, mask), True, padding_mask)
            x = x + self.drop_path(feat)
            x = x + self.drop_path(self.mlp(self.masknorm2(x, mask)))
            return x, attn
        else:
...
class MaskedLayerNorm(nn.Module):
    ...
    def forward(self, x, mask=None):
     ...
     #   std = x.std(-1, keepdim=True)
        #print(std)
     #   x_norm = (x - mean) / (std + self.norm.eps)
     #   print(self.norm.weight * x_norm + self.norm.bias)
        #return self.norm.weight * x_norm + self.norm.bias
        mask = mask.unsqueeze(-1)  # (batch_size, seq_len, 1)
        masked_x = x * mask

        # 计算有效元素数量
        count = x.shape[-1] # (batch_size, 1, hidden_dim)

        # 计算均值
        mean = masked_x.sum(dim=-1, keepdim=True) / (count)  # (batch_size, 1, hidden_dim)
      #  print(mean)
        #print(mean.shape)
    #    print("****************************")

        # 计算方差
        variance = ((masked_x - mean) ** 2 * mask).sum(dim=-1, keepdim=True) / (count)
      #  print(variance)

        # 归一化
        x_normalized = (masked_x - mean) / torch.sqrt(variance + self.norm.eps)
        #return self.norm.weight * x_norm + self.norm.bias

        # 恢复缩放和平移
        result = self.norm.weight * x_normalized + self.norm.bias
     #   print(result)

        # 恢复未掩码位置的值
        #output = result * mask + x * (1 - mask)
...
class VisionTransformer(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.word_embeddings = nn.Embedding(self.bins * self.range + 5, in_channel, padding_idx=self.bins * self.range+4, max_norm=1, norm_type=2.0)

        self.position_embeddings = nn.Embedding(
            4, in_channel)
        self.output_bias = torch.nn.Parameter(torch.zeros(self.bins * self.range + 5))
...
def vit_large_patch16_224(pretrained=False, bins=400, range=2, extension=3, prenum=7, **kwargs):
    ...
    return model
def vit_tiny_patch16_224(pretrained=False, bins=400, range=2, extension=3, prenum=7, **kwargs):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    """
    model_kwargs = dict(
        patch_size=16, embed_dim=192, depth=12, num_heads=3, bins=bins, range_time=range, extension=extension, prenum=prenum, **kwargs)
