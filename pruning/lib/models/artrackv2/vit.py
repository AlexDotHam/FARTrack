...
class VisionTransformer(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.word_embeddings = nn.Embedding(self.bins * self.range + 5, in_channel, padding_idx=self.bins * self.range + 4, max_norm=None, norm_type=2.0)
        reduction_factor = bins // (256 // 16)
       # self.spatial_reduction = nn.Conv2d(
     #       in_channels=self.embed_dim,
    #        out_channels=self.embed_dim,
      #      kernel_size=reduction_factor,
     #       stride=reduction_factor,
     #       groups=self.embed_dim  # 每个通道独立卷积，保持特征维度独立性
    #    )
    #    nn.init.zeros_(self.spatial_reduction.weight)
     #   nn.init.zeros_(self.spatial_reduction.bias)
...
# 获取block的唯一编号
        block_numbers = sorted(list(set([int(k.split('.')[1]) for k in block_keys])))
        last_three_block_nums = block_numbers[-3:]  # 获取最后三个block的编号
...
# 复制参数并创建新的extension层
    ...
    # 对最后三层block的参数，复制到extension中
            if k.startswith('blocks.'):
                block_num = int(k.split('.')[1])
                if block_num in last_three_block_nums:
            # 计算应该映射到哪个extension (0,1,2)
                    ext_index = last_three_block_nums.index(block_num)
            # 构建新的extension key
                    new_key = k.replace(f'blocks.{block_num}', f'extension.{ext_index}')
            # 复制参数到新的extension层
                    new_checkpoint[new_key] = v.clone()  # 使用clone确保是独立的副本
...
def vit_tiny_patch16_224(pretrained=False, bins=400, range=2, extension=3, **kwargs):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    """
    model_kwargs = dict(
        patch_size=16, embed_dim=192, depth=12, num_heads=3, bins=bins, range_time=range, extension=extension, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224_in21k', pretrained=pretrained, **model_kwargs)
    return model
