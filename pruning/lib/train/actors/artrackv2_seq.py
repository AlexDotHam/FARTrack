...
class ARTrackV2SeqActor(BaseActor):
    ...
    def batch_init(self, images, template_bbox, initial_bbox) -> dict:
        ...
        self.mask = torch.ones([z_crop_tensor.shape[0], 445, 445]) > 0
      #  self.mask[:, :, 0:245] = False
    ...
    def batch_track(self, img, gt_boxes, template, action_mode='max') -> dict:
      ...
      #  target_in_search = torch.cat(target_in_search_list, dim=0)
        ...
        mask_list.append(self.mask.clone())
     #   print(self.mask[0])
        outputs = self.net(template, x_crop, seq_input=pre_seq_output, head_type=None,
                           stage="batch_track", mask=self.mask)

        self.mask = outputs['mask']
      ...
      #  update_feat_list.append(update_feat.clone())
       ...
       #     'dz_feat': update_feat,
          ...
          #  'target_in_search': target_in_search,
            ...
            'mask': torch.tensor([item.cpu().detach().numpy() for item in mask_list], dtype=torch.bool),
        }
        return out
        ...
        for idx in range(np.max(num_frames)):
            ...
            if idx == 0:
             ...
             #   target_in_search_list.append(outputs['target_in_search'])
                ...
                mask_list.append(outputs['mask'].clone())
    ...
    def compute_sequence_losses(self, data):
     ...
     #   target_in_search = data['target_in_search'].reshape(-1, *data['target_in_search'].size()[2:])
        ...
        mask = data['mask'].reshape(-1, *data['mask'].size()[2:])
        ...
        outputs = self.net(template_images_for, search_images, seq_input=pre_seq, mask=mask, stage="forward_pass",)
       ...
       # score_loss = self.objective['l1'](score_real, iou)
        ...
        total_losses = loss_bb #+  score_loss * self.loss_weight['score_update']
